...
void caffe_gpu_interp2(const int channels,
  ...
  float rwidth_;
  if (height1 % 2 == 0) {
      rheight_ = (height2 > 1) ? static_cast<float>(height1) / (height2) : 0.f;
      rwidth_ = (width2 > 1) ? static_cast<float>(width1) / (width2) : 0.f;
  } else {
      rheight_ = (height2 > 1) ? static_cast<float>(height1 - 1) / (height2 - 1) : 0.f;
      rwidth_ = (width2 > 1) ? static_cast<float>(width1 - 1) / (width2 - 1) : 0.f;
  }


  const float rheight = rheight_;
  const float rwidth = rwidth_;
  
  // LOG(INFO) << "rheight: "<<rheight;
